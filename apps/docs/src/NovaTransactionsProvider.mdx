...
The **NovaTransactionsProvider** is the centerpiece of the `@tuwaio/nova-transactions` library. It's a comprehensive, all-in-one solution that orchestrates the entire transaction tracking UI experience in your dApp. 🎯

## What is NovaTransactionsProvider?

`NovaTransactionsProvider` is a powerful React component that combines all transaction tracking UI features into a single, easy-to-use instance. It acts as the central coordinator for:

-   🍞 **Smart toast notifications** with live updates.
-   📱 **An interactive modal** for detailed transaction progress tracking.
-   💼 **A wallet info modal** for displaying the connected account and transaction history.
-   🌐 **An internationalization (i18n) context** for all UI elements.
...
To use the provider, you first need to create the Pulsar store using the EVM adapter (in example) or any other and a custom hook, as shown in `txTrackingHooks.tsx`. This setup allows you to pass the store's state and functions to the NovaTransactionsProvider. Then, you can use the `NovaTransactionsProvider` as a standalone component in your root layout.
...
`NovaTransactionsProvider` uses `react-toastify` under the hood. You can pass any props for its `ToastContainer` directly to `NovaTransactionsProvider` to control its behavior.
...
| ------------------------ | ---------------------------------- | :------: | ------------------------------------------------------------------------ |
| `adapter`                | `TxAdapter or TxAdapter[]`         |    ✅    | Adapter or array of configured adapters similar to your Pulsar store.                 |
| `transactionsPool`       | `TransactionPool`                  |    ✅    | The current transaction pool from your Pulsar store.                     |
| `handleTransaction`      | `Function`                         |    ✅    | The handler function from your store to initiate transactions.           |
| `closeTxTrackedModal`    | `(txKey?: string) => void`         |    ✅    | A function from your store to close the tracking modal.                  |
...
> **Note**: All props for `ToastContainer` from `react-toastify` are also supported and can be passed directly to `NovaTransactionsProvider`.
